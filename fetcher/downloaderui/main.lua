...
function loadPlatforms()
    ...
    local command = "cd /storage/roms/ports/fetcher && python3 get_platforms.py > /tmp/platforms.json 2>&1"
    os.execute(command)
...
function downloadNextRom()
    ...
    local command = string.format("cd /storage/roms/ports/fetcher && python3 downloader.py '%s' '%s' > /tmp/bulk_download_result_%d.json 2>&1 &", 
...
function fetchRomList()
    ...
    local command = string.format("cd /storage/roms/ports/fetcher && python3 fetcher.py '%s' > /tmp/file_list.json 2>&1 &", platform.name)
    os.execute(command)
...
function startRomDownloadReal()
    ...
    if gameState.selectedFile <= #currentList then
        ...
        -- Execute real File download
        local command = string.format("cd /storage/roms/ports/fetcher && python3 downloader.py '%s' '%s' > /tmp/file_download_result.json 2>&1 &", platform.name, file.filename)
        os.execute(command)
